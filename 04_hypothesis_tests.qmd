...
[^3]: Eagle-eyed readers will notice that the null tested here is a point, where we previously defined the null in a one-sided test as a region $H_0: \theta \leq \theta_0$. Technically, the size of the test will vary based on which of these nulls we pick. In this example, notice that any null to the left of $\theta_0$ will result in a lower size. This means that the null at the boundary, $\theta_0$ will maximize the size of the test and so it is the most "conservative" null to investigate. Technically, we should define the size of a test as $\alpha = \sup_{\theta \in \Theta_0} \pi(\theta)$, 
...
text(x = 0, y = 0.55, "Retain")
text(x = 2.32, y = 0.55, "Reject", pos = 4)
text(x = -1, y = dnorm(-1), expression(P(T ~~ "|" ~~ theta[0])), pos = 2)
curve(dnorm(x), from = -4, to = 4, ylim = c(0, 0.6), bty = "n", las = 1, xlab = "T under an alternative", ylab = "", xaxt = "n", yaxt = "n", col = "grey80", lwd = 2, main = "Power")
...
text(x = 2.32, y = 0.55, "Reject", pos = 4)
text(x = -1, y = dnorm(-1), expression(P(T ~~ "|" ~~ theta[0])), pos = 2)
text(x = 1, y = dnorm(1, mean = 1), expression(P(T ~~ "|" ~~ theta[1])), pos = 3, col = "indianred")
...
#| label: fig-two-sided
#| echo: false
#| fig-cap: "Rejection regions for a two-sided test."
curve(dnorm(x), from = -4, to = 4, ylim = c(-.05, 0.5), bty = "n", las = 1, xlab = "T under the null hypothesis", ylab = expression(P(T ~~ "|" ~~ theta[0] )))
abline(h = 0)
...
text(x = 1.96, y = 0.45, "Reject", pos = 4)
text(x = -1.96, y = 0.45, "Reject", pos = 2)
text(x = 1.94, y = -0.04, "c", pos = 4)
text(x = -1.94, y = -0.04, "-c", pos = 2)
text(x = 2.2, y = 0.05, expression(1 - G[0](c)), pos = 4)
text(x = -2.2, y = 0.05, expression(G[0](-c)), pos = 2)
```
...
abline(v = qnorm(1 - 0.05), col = "grey30", lty = 2, lwd = 2)
abline(v = qt(1 - 0.05, df = 5), col = "dodgerblue", lty = 2, lwd = 2)
```
...
At first glance, we may seem sloppy in using $\alpha$ in the derivation of a $1 - \alpha$ confidence interval in the last chapter and in an $\alpha$-level test in this chapter. In reality, we were simply foreshadowing the deep connection between the two: every $1-\alpha$ confidence interval contains all null hypotheses that we **would not reject** with an $\alpha$-level test. 

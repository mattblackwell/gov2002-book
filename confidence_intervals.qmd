...
$$
where we have used the usual plugin estimator for the standard error. A lower bound of $0.511$ tells us that values below that are only compatible with very large and rare chance errors. What if a 2.5% chance of this bound being wrong is too high? We could instead consider what the truth would be for a positive chance error that is larger than 99% of chance errors, which would be $z_{0.99} \times \se[\widehat{\theta}_n] \approxeq 2.32 \times \se[\widehat{\theta}_n]$. In the immigration example, we have
$$
...
Consider the sample mean of an iid random variables, $X_1, \ldots, X_n$ where $\E[X_i] = \mu$ and $\V[X_i] = \sigma^2$. Recall that the central limit theorem shows that the sample mean will roughly be normally distributed $\Xbar_n \sim N(\mu, \sigma^2/n)$, which implies we can estimate its standard error as
$$
...
3. We want lower a confidence level ($\alpha$ is larger so $z_{1-\alpha/2}$ is smaller).
...
From this expression, we can see that the width of the difference-in-means confidence intervals are a function of the noise and number of observations in both samples. 
...
Consider a sample of binary random variables, $X_i \in \{0,1\}$ with a common probability of 1, $p = \P(X_i = 1) = \E[X_i]$. We can construct the sample proportion, $\widehat{p} = \frac{1}{n} \sum_{i=1}^n X_i$, which is exactly the same as the sample mean. Thus, we could also use the above asymptotically normal confidence intervals, and they do indeed work well in large samples. However, these normal-based CIs can perform poorly in small samples, especially when the population proportion, $p$, is close to 0 or 1. For example, suppose we obtain $\widehat{p} = 0.99$ with $n=100$. The normal-based 95% confidence interval would be
$$
\begin{aligned}
...
Let's use hypothesis test inversion. The relevant test is of the null hypothesis $H_0: p = p_0$. With binary outcomes, the null hypothesis determines the entire sampling distribution distribution of $\widehat{p}$ as $N(p_0, p_0(1-p_0)/n)$. The sampling variance for the sample proportion only depends on the population proportion, unlike the more general sample mean case where it depends on another unknown quantity, the population variance. We can then implement a test with the following rejection region:
$$
...
Solving this quadratic equation using your favorite mneonmic device gives the interval
$$
...
@fig-wilson-vs-wald compares the Wilson score interval to the asymptotic normal interval (often called the Wald interval in this context) as a function of the sample proportion for $n = 50$. The Wilson interval is always contained within $[0, 1]$, while the Wald interval extends above 1 when $\widehat{p}$ is close to 1. The Wilson interval is also notably wider than the Wald interval near the boundary, reflecting a more honest accounting of the uncertainty in this region.

```{r}
#| label: fig-wilson-vs-wald
#| echo: false
#| fig-cap: "Comparison of the 95% Wald (normal) and Wilson score confidence intervals as a function of the sample proportion with $n = 50$."
n <- 50
...
legend("topleft", legend = c("Wald", "Wilson"), col = c("indianred", "dodgerblue"),
       lwd = 2, bty = "n")
```

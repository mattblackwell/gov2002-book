...
Consider the sample mean of a iid random variables, $X_1, \ldots, X_n$ where $\E[X_i] = \mu$ and $\V[X_i] = \sigma^2$. Recall that the central limit theorem shows that the sample mean will roughly be normally distributed $\Xbar_n \sim N(\mu, \sigma^2/n)$, which implies we can estimate its standard error as
$$
\widehat{\se}[\Xbar] = \frac{S_n}{\sqrt{n}},
$$
where $S_n = (n-1)^{-1} \sum_{i=1}^n (X_i - \Xbar)^2$ is the sample variance. 
...
$$
CI_{\alpha, jk} = \left[\widehat{\theta}_n - z_{\alpha/2}\sqrt{V_{jk}},\; \widehat{\theta}_n + z_{\alpha/2}\sqrt{V_{jk}} \right] 
$$
...
1. Draw a sample with replacement of size $n$ from $F_n$ and apply the estimator to this sample, $\widehat{\theta}_(r)$.
2. Repeat Step 1 $R$ time to obtain $\widehat{\theta}^*_{(1)}, \widehat{\theta}^*_{(2)}, \ldots, \widehat{\theta}^*_{(R)}$. 

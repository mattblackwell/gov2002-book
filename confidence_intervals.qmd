...
3. Calculate the $\alpha/2$ and $1-\alpha/2$ quantiles of the distribution of $(\widehat{\theta}^*_{(1)}, \ldots, \widehat{\theta}^*_{(R)})$, which are the bootstrap estimates minus the full sample estimate, and define these as $\widetilde{\theta}^*_{\alpha/2}$ and $\widetilde{\theta}^*_{1-\alpha/2}$. 

...
curve(
  dnorm(x), from = -4, to = 4, ylim = c(-.05, 0.5), bty = "l", las = 1,
  xlab = "z", ylab = expression(f(z)), xaxt = "n"
)
abline(h = 0)
axis(
  side = 1, at = c(-1.96, 1.96),
  labels = c(expression(-z[alpha / 2]), expression(z[alpha / 2]))
)
polygon(
  c(-4, seq(-4, -1.96, 0.01), -1.96),
  c(0, dnorm(seq(-4, -1.96, 0.01)), 0),
  col = "grey70",
  border = NA
)
polygon(
...
low_bound <- up_bound <- rep(NA, times = sims)
for (i in 1:sims) {
  draws <- rnorm(500, mean = 1, sd = sqrt(10))
  low_bound[i] <- mean(draws) - sd(draws) / sqrt(500) * 1.96
  up_bound[i] <- mean(draws) + sd(draws) / sqrt(500) * 1.96
  if (low_bound[i] < 1 && up_bound[i] > 1) {
    cover[i] <- 1
  }
}
...
segments(x0 = 1:100, y0 = low_bound[1:100], y1 = up_bound[1:100],

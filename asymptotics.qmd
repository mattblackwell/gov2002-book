...
#| echo: false
#| fig-cap: |
#|  Sampling distribution of the sample mean as a function of
#|  sample size.
nsims <- 10000
...
curve(
  pnorm(x, 0.5, 1.5),
  from = -4, to = 6, add = TRUE, lty = 2, col = cols[2], lwd = 2
)
curve(
  pnorm(x, 0.25, 1.25),
  from = -4, to = 6, add = TRUE, lty = 3, col = cols[3], lwd = 2
)
curve(
  pnorm(x, 0.1, 1.1),
  from = -4, 6, add = TRUE, lty = 4, col = cols[4], lwd = 2
)
curve(
  pnorm(x, 0, 1), from = -4, 6, add = TRUE, lty = 1, col = "black", lwd = 2
)

curve(
  dnorm(x, 1, 2),
  from = -4, to = 6, bty = "n", yaxt = "n", ylab = "f(x)", main = "pdf",
  lwd = 2, col = cols[1], lty = 5, ylim = c(0, dnorm(0))
)
curve(
  dnorm(x, 0.5, 1.5),
  from = -4, to = 6, add = TRUE, lty = 2, lwd = 2, col = cols[2]
)
curve(
  dnorm(x, 0.25, 1.25),
  from = -4, to = 6, add = TRUE, lty = 3, lwd = 2, col = cols[3]
)
curve(
  dnorm(x, 0.1, 1.1),
  from = -4, to = 6, add = TRUE, lty = 4, lwd = 2, col = cols[4]
)
curve(
  dnorm(x, 0, 1), from = -4, to = 6, add = TRUE, lwd = 2, col = "black"
)
legend(
  x = "topright",
  legend = c("n=1", "n=2", "n=4", "n=10", "N(0,1)"),
  col = c(cols[1:4], "black"),
  lty = c(5, 2, 3, 4, 1),
  lwd = 2,
  bty = "n"
)
...
#| echo: false
#| fig-cap: |
#|  Sampling distributions of the normalized sample mean at
#|  n=30 and n=100.
set.seed(02138)
nsims <- 10000
...
par(las = 1, mar = c(3.1, 4.1, 0.1, 0.1))
plot(
  density((holder2[, 3] - 0.25) * sqrt(16 / 3) * sqrt(30)),
  xlim = c(-3, 3),
  bty = "n",
  main = "",
  lwd = 2,
...
lines(
  density((holder2[, 4] - 0.25) * sqrt(16 / 3) * sqrt(100)),
  bty = "n",
  main = "",
  lwd = 2,
  col = "indianred"
...
legend(
  x = 1,
  y = 0.5,
  legend = c("n = 30", "n = 100", "N(0, 1)"),
  col = c("dodgerblue", "indianred", "grey50"),
  lwd = 2,
  lty = c(1, 1, 2),
  bty = "n"
)
...
$$
but we actually want to estimate $h(\theta)$ so we use the plug-in estimator, $h(\widehat{\theta}_n)$. It seems like we should be able to apply part 1 of @thm-indist-properties to obtain the asymptotic distribution of $h(\widehat{\theta}_n)$. Still, the CLT established the large-sample distribution of the centered and scaled random sequence, $\sqrt{n}(\widehat{\theta}_n - \theta)$, not to the original estimator itself, and we would need the latter to investigate the asymptotic distribution of $h(\widehat{\theta}_n)$. We can use a little bit of calculus to get an approximation of the distribution we need. 
...
Understanding what is happening here provides intuition as to when this might go wrong. Why do we focus on continuously differentiable functions, $h()$? These functions can be well-approximated with a line in a neighborhood around a given point like $\theta$. In @fig-delta, we show this at the point where the tangent line at $\theta_0$, which has slope $h'(\theta_0)$, is very similar to $h(\theta)$ for values close to $\theta_0$. Because of this, we can approximate the difference between $h(\widehat{\theta}_n)$ and $h(\theta_0)$ with the what this tangent line would give us:
$$
...
curve(
  x ^ 2, from = -3, to = 3, lwd = 2, bty = "l", ylim = c(0, 10),
  xaxt = "n", yaxt = "n", xlab = expression(theta),
  ylab = expression(h(theta))
)
axis(
  side = 1, at = c(-1, -1.25),
  labels = c(expression(theta[0]), expression(hat(theta)[n]))
)
axis(
  side = 2, at = c(1, 1.25^2),
  labels = c(expression(h(theta[0])), expression(h(hat(theta)[n]))), las = 1
)
...
segments(
  x0 = -20, x1 = -1.25, y0 = 1.25 ^ 2, lwd = 1.5, lty = 2, col = "grey50"
)
...
text(
  x = -2.5,
  y = 3.5, expression(h * minute * (x)), pos = 2, cex = 0.88,
  offset = 0.1
)
